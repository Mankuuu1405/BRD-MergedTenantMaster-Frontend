...
export default function ActiveLoans() {
  ...
  const fetchActiveLoans = async () => {
    ...
    try {
      ...
      setLoans(Array.isArray(res.data) ? res.data : (res.data?.results || []));
    } catch (err) {
              ...
              {loading ? (
                ...
                filteredLoans.map((loan) => (
                  <tr key={loan.id} className="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="px-4 md:px-8 py-5">
                      <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border whitespace-nowrap ${loan.status === 'OVERDUE' ? 'bg-amber-50 text-amber-700 border-amber-200' :
