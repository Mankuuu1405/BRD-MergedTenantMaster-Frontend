...
export default function Users() {
  ...
  const roleMap = useMemo(() => (Array.isArray(roles) ? roles : []).reduce((acc, role) => { acc[role.id] = role.name; return acc; }, {}), [roles]);
  ...
  const stats = useMemo(() => ({
    ...
    active: (Array.isArray(users) ? users : []).filter(u => u.is_active || u.status === 'Active').length,
