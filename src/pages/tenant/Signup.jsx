...
export default function Signup() {
  ...
  useEffect(() => {
    ...
    // fetchProducts(); // Mocking for now to avoid errors if API fails
  }, []);
        ...
        {isOpen && (
          <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-56 overflow-y-auto">
            {options.map((option) => (
              ...
              >
                <div className={`w-5 h-5 rounded border flex items-center justify-center shrink-0 ${selected.includes(option.product_name) ? "bg-blue-600 border-blue-600" : "border-gray-300 bg-white"
                  }`}>
